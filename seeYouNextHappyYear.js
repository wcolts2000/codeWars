...
function nextHappyYear(year) {
  let answer = year + 1;
  let yearStr = `${answer}`.split('');
  let uniqueYearString = [...new Set(yearStr)];
  while (uniqueYearString.length < yearStr.length) {
    answer++;
    yearStr = `${answer}`.split('');
    uniqueYearString = [...new Set(yearStr)];
  }
  return answer;
}
